...
from competency_system.domain.entities import RankingSnapshot, Vacancy
...
async def test_ranking_snapshot_repository_get_by_vacancy_and_upsert(
    ...
    assert loaded.payload["rankings"][0]["candidate"] == "cand-1"

    snapshot.payload = {"rankings": [{"candidate": "cand-2"}]}
    await repo.add(snapshot)
    await pg_session.commit()

    updated = await repo.get_by_vacancy(vacancy.id)
    assert updated is not None
    assert updated.payload["rankings"][0]["candidate"] == "cand-2"
