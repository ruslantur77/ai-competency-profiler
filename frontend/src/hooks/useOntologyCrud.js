...
export function useOntologyCrud({
  ...
  const handleCreateSubCompetency = useCallback(
    async (payload) => {
  ...
  const handleUpdateSubCompetency = useCallback(
    async (payload) => {
      try {
  ...
  const openCreateCategory = useCallback(() => {
    setEditingCategory({
      mode: 'create',
      payload: { id: '', name: '', emoji: '📋', description: '' },
    });
  }, []);
  ...
  const openDeleteSelected = useCallback(() => {
    ...
    const label =
      selected.type === ENTITY_TYPE.CATEGORY
