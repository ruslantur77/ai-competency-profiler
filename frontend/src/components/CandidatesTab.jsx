...
export default function CandidatesTab({ notify, vacancies, onOpenVacancy, onOpenRanking }) {
  ...
  useEffect(() => {
    ...
    const loadDetail = async () => {
      ...
      try {
        ...
        setCandidateDetail(null);
        setCandidateProfile(null);
        notify(getErrorMessage(error, { fallback: 'Ошибка загрузки профиля кандидата' }), 'error');
      } finally {
  ...
  const handleDeleteCandidate = async () => {
    ...
    try {
      ...
      const lastValidOffset =
        nextTotal > 0 ? Math.floor((nextTotal - 1) / pageLimit) * pageLimit : 0;
  ...
  const selectedVacancy = selectedCandidate ? vacancyMap.get(selectedCandidate.vacancy_id) : null;
          ...
          {detailLoading ? (
            ...
            <>
              ...
              <div className="candidates-tab__detail-meta">
                <span>Статус: {selectedStatusMeta?.label || candidateDetail.status}</span>
                <span>Последняя оценка: {formatDateTime(candidateDetail.last_assessment_at)}</span>
                <span>Вакансия: {selectedVacancy?.name || candidateDetail.vacancy_id}</span>
              </div>
              ...
              <div className="candidates-tab__actions">
                ...
                >
                  <BarChart3 size={14} />В ранжирование
                </button>
                <button className="btn-danger" onClick={() => setConfirmDelete(candidateDetail)}>
                  <Trash2 size={14} />
                  Удалить
                </button>
              </div>
