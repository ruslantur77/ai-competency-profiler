...
2. По пилотному набору вакансий система корректно выделяет все обязательные компетенции в 9 из 10 случаев. Если ниже - пилот считается провальным.
3. Подготовка профиля вакансии с правками эксперта занимает существенно меньше времени по сравнению с полностью ручным составлением.
...
**Подзадача 1 - Извлечение компетенций из текста вакансии (LLM structured output)**
...
**Подзадача 2 - Маппинг нод курса на онтологию компетенций (LLM classification)**

Вход: граф курса из LMS (ноды с названиями, типами, тегами, текстом лекций, описаниями задач).
Выход: для каждой ноды - список подкомпетенций из онтологии с весом присутствия (0–1), отражающим степень раскрытия подкомпетенции в данной ноде.
Техника: та же трёхступенчатая схема, что и в подзадаче 1, применённая к каждой ноде курса. Результат валидируется экспертом; после валидации маппинг фиксируется и используется для скоринга.
...
- Размеченный набор из не менее 5 курсов с валидированными маппингами нод (ground truth для подзадачи 2).
...
| Маппинг нод            | Agreement с экспертом: доля нод с корректным маппингом          | Фиксируем как отправную точку |
...
Трёхступенчатый LLM-пайплайн аналогичной архитектуры, применённый к каждой ноде курса. Модель получает содержимое ноды (название, теги, текст лекции или описание задачи) и список подкомпетенций текущего уровня из онтологии, определяет релевантные и присваивает вес присутствия (0.0–1.0). Результат валидируется экспертом и фиксируется в БД.
