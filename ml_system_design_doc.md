...
**MVP подзадача 2 (извлечение компетенций):**
...
- LLM-оценки кода сравниваются с эталонной экспертной разметкой на наборе решений.
...
- Результаты используются для оценки бизнес-метрики (доля успешных интервью из топ-N) и качественного анализа ошибок ранжирования. При накоплении достаточного объёма данных (следующие итерации) - для калибровки весов скорингового модуля
...
| Корреляция LLM-оценки кода с эталонной разметкой            | Сравнение с ground truth                       | Значимо выше бейзлайна (p < 0.05) |
...
Система состоит из трёх независимых сервисов, взаимодействующих через REST API и асинхронную очередь задач.

```mermaid
flowchart TD
    subgraph Клиенты
        HR[HR-менеджер браузер]
        EXP[Технический эксперт браузер]
    end

    subgraph Внешние системы
        EXT[Внешняя тестирующая система: задания, результаты, код]
    end

    subgraph Наша система
        GW[API Gateway - аутентификация, роутинг]
        APP[Backend API - бизнес-логика]
        LLM_W[LLM Worker - асинхронная обработка]
        DB[(PostgreSQL - онтология, графы, маппинги, результаты)]
        QUEUE[Очередь задач - Celery + Redis]
        LLM[Внешний LLM-провайдер - gpt-oss-20b]
    end
    ...
    LLM_W --> LLM
    LLM_W --> DB
    EXT -->|webhook / polling - результаты тестирования| APP
    APP -->|запрос заданий| EXT
```

**Сервисы и их назначение:**

- **API Gateway** - аутентификация по JWT, роутинг запросов, rate limiting.
- **Backend API** - основная бизнес-логика: управление вакансиями, графами компетенций, маппингами, ранжированием. Синхронные операции (CRUD, чтение рейтинга) обрабатываются здесь напрямую.
- **LLM Worker** - асинхронный воркер для всех LLM-задач: трёхступенчатый пайплайн по вакансии, маппинг заданий, оценка кода. Получает задачи из очереди, вызывает внешний LLM-провайдер, записывает результаты в БД.
- **PostgreSQL** - основное хранилище: онтология компетенций, графы вакансий, маппинги заданий, векторы компетенций кандидатов, результаты ранжирования.
- **Celery + Redis** - очередь задач для асинхронной обработки LLM-запросов и интеграции с тестирующей системой.


### 4.2. Описание инфраструктуры и масштабируемости

**Выбор: облачный деплой на одном инстансе с возможностью горизонтального масштабирования воркеров.**

Для MVP все компоненты разворачиваются на одном облачном сервере. LLM Worker выделен в отдельный процесс/контейнер - это позволяет масштабировать его независимо при росте нагрузки без изменения архитектуры.

| Компонент   | MVP                 | При росте нагрузки                                |
| ----------- | ------------------- | ------------------------------------------------- |
| Backend API | 1 инстанс           | Горизонтальное масштабирование за балансировщиком |
| LLM Worker  | 1–2 воркера         | Увеличение числа воркеров, батчинг запросов       |
| PostgreSQL  | Managed DB (облако) | Read replicas для аналитических запросов          |
| Redis       | Managed Redis       | Кластерный режим                                  |

**Плюсы выбора:**

- Простота деплоя и поддержки на этапе MVP.
- LLM Worker масштабируется независимо - основной источник нагрузки изолирован.
- Managed-сервисы БД снимают операционную нагрузку.
- Celery позволяет добавлять приоритеты задач (оценка кода vs маппинг заданий) без изменения архитектуры.

**Минусы выбора:**

- На MVP нет автоматического масштабирования - при пиковой нагрузке возможны задержки в очереди.
- Зависимость от внешнего LLM-провайдера: недоступность провайдера блокирует все асинхронные задачи.

**Почему лучше альтернатив:** Serverless-архитектура (Yandex Cloud / Cloud.ru) была бы проще в масштабировании, но плохо подходит для длинных LLM-цепочек (таймауты, сложность отладки). Монолит без очереди задач не даёт изолировать LLM-нагрузку. Выбранная схема - минимально сложная для MVP и не требует переработки при росте.
...
| Операция                                    | Ожидаемая задержка | Комментарий                                  |
| ------------------------------------------- | ------------------ | -------------------------------------------- |
| Загрузка текста вакансии и запуск пайплайна | < 2 с              | Задача ставится в очередь, ответ немедленный |
| Формирование черновика графа компетенций    | < 3 мин            | Асинхронно, эксперт получает уведомление     |
| Маппинг одного задания                      | < 2 мин            | Асинхронно, при добавлении задания в систему |
| Оценка кода одного решения                  | < 30 с             | Асинхронно, после завершения тестирования    |
| Формирование рейтинга кандидатов            | < 5 с              | Синхронно, по запросу HR                     |
| Чтение профиля компетенций кандидата        | < 1 с              | Синхронно, из БД                             |

**Пропускная способность (оценка для MVP):**

- До 10 активных вакансий одновременно.
- До 500 кандидатов на вакансию.
- До 50 заданий в тестовом наборе.
- Пиковая нагрузка: завершение тестирования потоком кандидатов - до 100 решений в очереди на оценку кода одновременно. При 2 воркерах и ~30 с на решение - очередь разбирается за ~25 мин.
...
- **Prompt injection в LLM-запросах.** Текст вакансии или описание задания, переданные в промпт, могут содержать инструкции для модели. Решение: строгая схема JSON-вывода с валидацией; системный промпт изолирован от пользовательского контента.
- **Несанкционированный доступ к результатам тестирования.** Один HR не должен видеть данные другой компании. Решение: изоляция данных по id на уровне БД и API; JWT содержит идентификатор компании.
- **Перебор одноразовых ссылок для кандидатов.** Решение: ссылки генерируются как криптографически случайные UUID; ограничение числа попыток активации.
- **Злоупотребление LLM-оценкой кода.** Кандидат может попытаться передать в код инструкции для модели. Решение: промпт явно инструктирует модель оценивать только технические критерии и игнорировать любые инструкции внутри кода.


### 4.5. Безопасность данных

**Персональные данные кандидатов:** Персональные данные кандидата (имя, email) хранятся только во внешней тестирующей системе и привязаны к одноразовой ссылке. В нашу систему передаётся только анонимный ID кандидата и артефакты тестирования (код, ответы, попытки). Таким образом, наша система не является оператором персональных данных.

**Код кандидата и LLM-провайдер:** Код решений передаётся во внешний LLM-провайдер для оценки. Код не содержит явных персональных данных (передаётся по анонимному ID). При выборе провайдера необходимо убедиться в наличии соглашения об обработке данных (DPA) и в том, что данные не используются для дообучения модели. В перспективе - переход на self-hosted LLM с OpenAI-совместимым интерфейсом, что полностью исключает передачу данных третьим сторонам.

**Данные компаний:** Тексты вакансий и описания заданий могут содержать конфиденциальную информацию компании. Передаются в LLM-провайдер в рамках того же DPA. При переходе на self-hosted - риск устраняется.


### 4.6. Издержки

**Расчёт операционных издержек в месяц (оценка для MVP):**

Предположения: 20 вакансий в месяц, 30 заданий в банке (обновляются редко, маппинг разовый), 300 кандидатов в месяц с оценкой кода по 3 задания каждый.

| Статья                                               | Расчёт                             | Стоимость/мес   |
| ---------------------------------------------------- | ---------------------------------- | --------------- |
| LLM: пайплайн вакансий (20 × ~17 500 input т.)       | ~350K input + ~30K output токенов  | ~$0.015         |
| LLM: маппинг заданий (30 × ~18 300 input т., разово) | ~550K input + ~5K output токенов   | ~$0.017         |
| LLM: оценка кода (300 × 3 × ~1 100 input т.)         | ~990K input + ~180K output токенов | ~$0.055         |
| Облачный сервер (2 vCPU, 4 GB RAM)                   | -                                  | ~$20–30         |
| Managed PostgreSQL                                   | -                                  | ~$15–20         |
| Managed Redis                                        | -                                  | ~$5–10          |
| **Итого**                                            |                                    | **~$40–65/мес** |

LLM-затраты незначительны относительно инфраструктурных. Основная статья расходов - облачная инфраструктура. При переходе на self-hosted LLM инфраструктурные расходы вырастут, но LLM-затраты обнулятся.


### 4.7. Integration points

**Интеграция с внешней тестирующей системой:**

Система взаимодействует с тестирующей платформой по двум направлениям:

- **Получение заданий** (при добавлении задания в банк): тестирующая система отправляет webhook с описанием задания (название, описание, тип, теги, тест-кейсы). Backend API принимает, ставит задачу маппинга в очередь LLM Worker.
- **Получение результатов тестирования** (после завершения теста кандидатом): тестирующая система отправляет webhook с результатами (анонимный ID кандидата, ID задания, ответы, код, попытки, статус тест-кейсов). Backend API принимает, запускает детерминированный скоринг и ставит задачи оценки кода в очередь.

| Метод                            | Направление                 | Описание                                 |
| -------------------------------- | --------------------------- | ---------------------------------------- |
| POST /webhooks/task-added        | Тестирующая система -> наша | Новое задание добавлено в банк           |
| POST /webhooks/results           | Тестирующая система -> наша | Результаты тестирования кандидата        |
| GET /api/vacancies/{id}/ranking  | HR -> наша                  | Получить рейтинг кандидатов              |
| GET /api/candidates/{id}/profile | HR/эксперт -> наша          | Профиль компетенций кандидата            |
| POST /api/vacancies              | HR -> наша                  | Создать вакансию, запустить LLM-пайплайн |
| PATCH /api/vacancies/{id}/graph  | Эксперт -> наша             | Отредактировать граф компетенций         |
...
| ------------------------------------------------ | -------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Внешний LLM-провайдер недоступен                 | Средняя        | Высокое: все асинхронные задачи встают      | Retry с экспоненциальным backoff; задачи не теряются в очереди; алерт при деградации                                     |
| Изменение API или ценообразования LLM-провайдера | Средняя        | Среднее                                     | Интерфейс к LLM изолирован в отдельном модуле; переход на другого провайдера или self-hosted без изменения бизнес-логики |
| Webhook от тестирующей системы не доставлен      | Низкая         | Среднее: результаты кандидата не обработаны | Подтверждение получения (200 OK); повторная отправка на стороне тестирующей системы; polling как fallback                |
| Рост нагрузки превышает мощность воркеров        | Низкая для MVP | Среднее: задержка результатов               | Мониторинг длины очереди; горизонтальное масштабирование воркеров                                                        |
| Утечка данных компании через LLM-провайдера      | Низкая         | Высокое                                     | DPA с провайдером; переход на self-hosted в production                                                                   |
